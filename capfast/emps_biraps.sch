[schematic2]
uniq 12
[tools]
[detail]
w 1668 939 100 0 n#1 trcalcos.$(PS):STATUS.INPL 1800 936 1536 936 1536 1840 1112 1840 inhier.enable.P
w 1532 1355 100 0 n#2 trcalcos.$(PS):STATCOMM.FLNK 1496 1352 1568 1352 1568 904 1800 904 trcalcos.$(PS):STATUS.SLNK
w 1548 1323 100 0 n#3 trcalcos.$(PS):STATCOMM.VAL 1496 1320 1600 1320 1600 1064 1800 1064 trcalcos.$(PS):STATUS.INPH
w 2124 1131 100 0 n#4 trcalcos.$(PS):STATUS.FLNK 2088 1128 2160 1128 2160 1384 1816 1384 1816 1512 1944 1512 ecalcos.$(PS):DRVDISABLE.SLNK
w 1792 1899 100 0 n#1 ecalcos.$(PS):DRVDISABLE.INPA 1944 1896 1640 1896 1640 1760 1536 1760 junction
w 3972 1627 100 0 n#5 ecalcos.$(PS):CURDISABLE.FLNK 3832 1624 4112 1624 4112 1488 4272 1488 ecalcos.$(PS):CURTRIP.SLNK
w 4608 1715 100 0 n#6 ecalcos.$(PS):CURTRIP.FLNK 4560 1712 4656 1712 outhier.flink.p
w 4588 1683 100 0 n#7 ecalcos.$(PS):CURTRIP.VAL 4560 1680 4616 1680 4616 1640 4680 1640 outhier.statok.p
w 2820 1739 100 0 n#8 ecalcos.$(PS):DRVDISABLE.FLNK 2232 1736 3408 1736 3408 1400 3544 1400 ecalcos.$(PS):CURDISABLE.SLNK
w 232 1051 100 0 n#9 inhier.scanin.P 184 1048 280 1048 280 912 376 912 esels.$(PS):LOCKSEL.SLNK
w 924 1155 100 0 n#10 esels.$(PS):LOCKSEL.FLNK 664 1152 1184 1152 1184 1128 1208 1128 trcalcos.$(PS):STATCOMM.SLNK
w 892 1123 100 0 n#11 esels.$(PS):LOCKSEL.VAL 664 1120 1120 1120 1120 1032 1800 1032 trcalcos.$(PS):STATUS.INPI
n 2619 1971 3086 2043 100
FUNCTIONALITY:
CHECK THAT BiRa DEVICE OPTICS ELEMENT IS ON,
COMMUNICATING AND NOT IN TIMEOUT STATE.
_
n 2217 1322 2538 1418 100
LED RULES FOR $(D)$(PS):STATUS:
0: PATH BAD -> RED
1: PATH GOOD -> GREEN
2: DISABLED -> BLACK
_
n 2065 1162 2396 1210 100
Problem with previous OCAL
L?!H?3:B?2:!A?1:(!(C|D|E|F|G)?5:0):4
_
n 2812 1987 3215 2057 150
Functionality:
Please add functionality here
_
[cell use]
use bb200tr 712 632 100 0 bb200tr#2
xform 0 1992 1432
p 2296 824 150 0 -1 seta:M. Rowe
p 2152 1016 150 0 -1 sets:E-Linac Control System
p 2152 984 100 0 -1 sett0:Machine Protect System
p 2152 952 100 0 -1 sett1:Optics Subsystem - Stream Device PS Status
p 2568 824 150 0 -1 setdat:2016/08/15
p 2328 872 200 0 -1 setd:$Revision: 1.1 $
p 2168 760 150 0 -1 setf:emps_biraps.sch
use trcalcos 1884 795 100 0 $(PS):STATUS
xform 0 1944 1096
p 1828 795 100 0 -1 PV:$(D)
p 1633 1282 100 0 -1 def(INPA):$(PS):STATON
p 1622 1249 100 0 -1 def(INPB):$(PS):STATTMO
p 1644 1214 100 0 -1 def(INPC):$(PS):FAULT
p 1631 1183 100 0 -1 def(INPD):$(PS):INTFLT0
p 1880 1256 150 0 1 CALC:L?((A&!(B&C&D&E&F&G))?1:0):2
p 1879 1235 100 0 1 OCAL:L?!H?3:B?2:!A?1:I:4
p 2094 1059 100 0 1 def(OUT):$(D)$(PS):STATTEXT PP
p 1880 1000 100 0 1 OOPT:Every Time
p 1878 978 100 0 1 DOPT:Use OCAL
p 1635 1153 100 0 -1 def(INPE):$(PS):INTFLT1
p 1623 1119 100 0 -1 def(INPF):$(PS):KLIXFLT0
p 1625 1086 100 0 -1 def(INPG):$(PS):KLIXFLT1
p 1590 1058 100 0 0 def(INPH):
p 1565 1024 100 0 0 def(INPI):
p 1624 993 100 0 0 def(INPJ):
use embbis 2730 1192 100 0 $(PS):STATTEXT
xform 0 2824 1256
p 2766 1158 100 0 1 ONVL:1
p 2769 1141 100 0 1 TWVL:2
p 2771 1124 100 0 1 THVL:3
p 2856 1176 100 0 1 ZRST:OK
p 2851 1158 100 0 1 ONST:Off
p 2855 1139 100 0 1 TWST:Timeout
p 2857 1123 100 0 1 THST:Comm.
p 2771 1178 100 0 1 ZRVL:0
p 2674 1192 100 0 -1 PV:$(D)
p 2773 1107 100 0 1 FRVL:4
p 2861 1105 100 0 1 FRST:Not Monitored
p 2772 1091 100 0 1 FVVL:5
p 2859 1088 100 0 1 FVST:Water/Temp.
p 2773 1075 100 0 1 SXVL:6
p 2858 1072 100 0 1 SXST:Locked
use inhier 147 1072 100 0 scanin
xform 0 184 1048
use outhier 4627 1742 100 0 flink
xform 0 4640 1712
use inhier 1077 1862 100 0 enable
xform 0 1112 1840
use outhier 4647 1657 100 0 statok
xform 0 4664 1640
use trcalcos 1273 1035 100 0 $(PS):STATCOMM
xform 0 1352 1320
p 1217 1035 100 0 -1 PV:$(D)
p 983 1505 100 0 -1 def(INPA):$(PS):STATON.STAT
p 971 1475 100 0 -1 def(INPB):$(PS):CURRDBK.STAT
p 994 1441 100 0 -1 def(INPC):$(PS):RDVOL.STAT
p 894 1409 100 0 -1 def(INPD):$(PS):RDUNSIGNEDCUR.STAT
p 992 1378 100 0 -1 def(INPE):$(PS):FAULT.STAT
p 993 1346 100 0 -1 def(INPF):$(PS):INTFLT0.STAT
p 974 1311 100 0 -1 def(INPG):$(PS):INTFLT1.STAT
p 987 1279 100 0 -1 def(INPH):$(PS):KLIXFLT0.STAT
p 1288 1480 150 0 1 CALC:!(A&&B&&C&&D&&E&&F&&G&&H&&I)
p 1292 1460 100 0 1 OCAL:0
p 989 1245 100 0 -1 def(INPI):$(PS):KLIXFLT1.STAT
use ecalcos 1991 1412 100 0 $(PS):DRVDISABLE
xform 0 2088 1704
p 1935 1412 100 0 -1 PV:$(D)
p 2024 1864 150 0 1 CALC:A&(B=1)
p 2248 1668 100 0 1 def(OUT):$(PS):DRVNOCHANGE.DISA
p 2028 1844 100 0 1 OCAL:0
p 1764 1857 100 0 -1 def(INPB):$(D)$(PS):STATUS
p 2024 1608 100 0 1 OOPT:On Change
use ecalcos 4319 1388 100 0 $(PS):CURTRIP
xform 0 4416 1680
p 4263 1388 100 0 -1 PV:$(D)
p 4352 1840 150 0 1 CALC:A?(ABS(B-C)<(0.005*ABS(D-E))):F
p 4580 1641 100 0 0 def(OUT):
p 4356 1820 100 0 1 OCAL:0
p 4077 1836 100 0 1 def(INPB):$(PS):CUR
p 4352 1584 100 0 1 OOPT:On Change
p 4051 1801 100 0 1 def(INPC):$(PS):RDCUR
p 4019 1767 100 0 1 def(INPD):$(PS):CUR.DRVH
p 4026 1738 100 0 1 def(INPE):$(PS):CUR.DRVL
p 4011 1704 100 0 1 def(INPF):$(D)$(PS):STATUS
p 3969 1866 100 0 1 def(INPA):$(D)$(PS):CURDISABLE
p 4160 1776 100 0 0 def(FLNK):
p 4324 1524 100 0 0 SCAN:Passive
use ecalcos 3591 1300 100 0 $(PS):CURDISABLE
xform 0 3688 1592
p 3535 1300 100 0 -1 PV:$(D)
p 3629 1750 150 0 1 CALC:(A>2)&B&C
p 3852 1553 100 0 1 def(OUT):$(PS):CUR.DISP PP MS
p 3628 1732 100 0 1 OCAL:0
p 3275 1747 100 0 1 def(INPB):$(D)$(PS):STATUS
p 3624 1496 100 0 1 OOPT:On Change
p 3171 1711 100 0 1 def(INPC):EMPS:BMMODEGRANTSTATUS
p 3251 1686 100 0 0 def(INPD):
p 3287 1648 100 0 0 def(INPE):
p 3298 1615 100 0 0 def(INPF):
p 3628 1468 100 0 0 DOPT:Use CALC
p 3267 1783 100 0 1 def(INPA):EMPS:BMPROPREQ
use esels 430 839 100 0 $(PS):LOCKSEL
xform 0 520 1104
p 374 839 100 0 -1 PV:$(D)
p 263 1290 100 0 1 def(INPA):0
p 263 1258 100 0 1 def(INPB):6
p 84 1318 100 0 1 def(NVL):$(D)$(PS):CURDISABLE
p 264 1225 100 0 0 def(INPC):0
[comments]
